...
export const processTokenTransfer = async (connection: Connection, sender: Keypair, recipientInfo: IRecipientInfo, mint: PublicKey, decimals: number): Promise<string> => {
  ...
  const amount = new BN(recipientInfo.amount * 10e9).mul(new BN(10).pow(new BN(decimals))).div(new BN(10e9)).toString();

  const ix = createTransferCheckedInstruction(senderAta, mint, recipientAta.address, sender.publicKey, BigInt(amount), decimals);
